...
# ---------------------------------------------------------------------------
# Campaign additions: the remaining pre-design baselines B7-B11.
# These are HEURISTICS with no derivation.  Their role is to make the derived
# method prove that its complexity buys something (METHOD_NOVELTY_SEPARATION).
# ---------------------------------------------------------------------------

class OvershootPolicy(BasePolicy):
    """B7: ``rho_j`` from alarm severity alone (F04).

    ``rho_j = rho_hi / (1 + a * max(overshoot, 0))`` -- higher overshoot, less
    reuse.  Two scalars.
    """

    policy_class = IMPLEMENTABLE

    def __init__(self, m: int, rho_hi: float, a: float, k: int | None = None) -> None:
        self.m, self.k = int(m), int(m if k is None else k)
        self.rho_hi, self.a = float(rho_hi), float(a)
        self.max_m = self.m
        self.name = f"overshoot(m={self.m},rho_hi={self.rho_hi:g},a={self.a:g})"
        super().__init__()

    def decide(self, obs: CycleObservation) -> Decision:
        o = np.maximum(obs.overshoot, 0.0)
        return self._full(obs, self.rho_hi / (1.0 + self.a * o), self.m, self.k)


class WindowDispersionPolicy(BasePolicy):
    """B8: ``rho_j`` from the terminal-window sample variance alone (F07).

    Expected to be near-useless (the window variance estimates the innovation
    variance, which is 1 by construction, not the reference error).  Included
    because a measured negative is informative.
    """
    ...
    def __init__(self, m: int, rho_hi: float, a: float, k: int | None = None) -> None:
        ...
        self.rho_hi, self.a = float(rho_hi), float(a)
        self.max_m = self.m
        self.name = f"windisp(m={self.m},rho_hi={self.rho_hi:g},a={self.a:g})"
        super().__init__()

    def decide(self, obs: CycleObservation) -> Decision:
        w = np.minimum(self.m, obs.tau)
        cols = np.arange(obs.window.shape[1])[None, :]
        take = (cols < w[:, None]) & obs.window_valid
        n = np.maximum(w, 1)
        mean = np.where(take, obs.window, 0.0).sum(axis=1) / n
        sq = np.where(take, (obs.window - mean[:, None]) ** 2, 0.0).sum(axis=1)
        var = sq / np.maximum(n - 1, 1)
        return self._full(obs, self.rho_hi / (1.0 + self.a * var), self.m, self.k)


class CappedReusePolicy(BasePolicy):
    """B10: fixed ``rho`` with a cap on *consecutive* high-reuse cycles.

    Limits compounding by forcing a full refresh after ``n_max`` reuse cycles.
    Uses only the policy's own history (F13).
    """

    policy_class = IMPLEMENTABLE

    def __init__(self, m: int, rho: float, n_max: int, k: int | None = None) -> None:
        self.m, self.k = int(m), int(m if k is None else k)
        self.rho, self.n_max = float(rho), int(n_max)
        self.max_m = self.m
        self.name = f"capped(m={self.m},rho={self.rho:g},n_max={self.n_max})"
        self._run = None
        super().__init__()

    def reset(self, n_rep: int) -> None:
        self._run = np.zeros(n_rep, dtype=np.int64)

    def decide(self, obs: CycleObservation) -> Decision:
        r = obs.rep
        force = self._run[r] >= self.n_max
        rho = np.where(force, 0.0, self.rho)
        self._run[r] = np.where(force, 0, self._run[r] + 1)
        return self._full(obs, rho, self.m, self.k)


class ConfidenceGatedPolicy(BasePolicy):
    """B11: a gate, not a dial -- reuse only when ``|zbar_j|`` is small.

    ``rho_j = rho_hi`` if ``|zbar_j| <= q``, else ``0``.  This is ``B6`` with
    the low level pinned at zero and is the crudest possible use of the
    high-gain sensor; SAW must beat it.
    """

    policy_class = IMPLEMENTABLE

    def __init__(self, m: int, rho_hi: float, q: float, k: int | None = None) -> None:
        self.m, self.k = int(m), int(m if k is None else k)
        self.rho_hi, self.q = float(rho_hi), float(q)
        self.max_m = self.m
        self.name = f"confgate(m={self.m},rho_hi={self.rho_hi:g},q={self.q:g})"
        super().__init__()

    def decide(self, obs: CycleObservation) -> Decision:
        zbar = obs.zbar(self.m)
        rho = np.where(np.abs(zbar) <= self.q, self.rho_hi, 0.0)
        return self._full(obs, rho, self.m, self.k)


class OracleShiftAwarePolicy(BasePolicy):
    """Z4: knows ``Delta`` and steers the reference AWAY from the blind spot.

    Reads F14 and F18.  The blind spot of ledger S10 is ``|e - Delta|`` small;
    an oracle that knows both can refuse any update that would land there.
    NEVER deployable -- it exists to price ``H8`` (the blind spot has no
    implementable proxy).
    """

    policy_class = ORACLE

    def __init__(self, m: int, rho: float, guard: float, k: int | None = None) -> None:
        self.m, self.k = int(m), int(m if k is None else k)
        self.rho, self.guard = float(rho), float(guard)
        self.max_m = self.m
        self.name = f"oracle_shift(m={self.m},rho={self.rho:g},guard={self.guard:g})"
        super().__init__()

    def decide(self, obs: CycleObservation) -> Decision:
        if not isinstance(obs, OracleObservation):
            raise TypeError("OracleShiftAwarePolicy requires an OracleObservation")
        rbar = obs.e_current + obs.zbar(self.m)
        would_be = self.rho * rbar
        bad = np.abs(would_be - obs.shift) < self.guard
        rho = np.where(bad, 0.0, self.rho)
        return self._full(obs, rho, self.m, self.k)
