...
METRIC_LAYER.update({"FreshProp": COST, "Down": COST, "Eff": COST,
                     "Q95e": LATENT})


def fresh_proportional(res) -> np.ndarray:
    """Sensitivity cost model: ``(1 - rho_j) k_j`` fresh observations per alarm.

    The PRIMARY model is ``fresh_per_alarm`` (``C_fresh = k_j`` whenever any
    fresh baseline is collected, i.e. whenever ``rho_j < 1``).  This variant is
    reported as a declared sensitivity only (SAFETY_OBJECTIVES.md section 3.3
    sub-decision C-b).
    """
    return ((1.0 - res.post(res.rho)) * res.post(res.k)).mean(axis=1)


def downtime_per_alarm(res) -> np.ndarray:
    """Unmonitored post-alarm observations per alarm under the blind-window model.

    Equal to ``Fresh`` under the default model; reported separately because it
    is an operational *risk* term, not only a cost term.
    """
    return fresh_per_alarm(res)
