...
class GlobalBudget:
    ...
    def status(self) -> dict:
        ...
        return {"cap": cap,
                "completed_cells": sorted(int(c) for c in state.get("completed_cells", {})),
                "remote_completed_cells": sorted(
                    int(c) for c in state.get("remote_completed_cells", {})),
                "remote_handoff_payload_hash": state.get("remote_handoff_payload_hash")}

    def import_remote_completion(self, *, role: str, cell_ids, finalized_cpu_h,
                                 payload_hash: str) -> dict:
        """Seed this host's ledger with ANOTHER host's finished shard.

        The imported remote completion is IMMUTABLE: a second import carrying a
        different payload hash is refused rather than overwriting, so accounting
        can never be reset or rolled backward. It seeds committed cost only; the
        in-flight reservation arithmetic is untouched.
        """
        amount = self._validate(finalized_cpu_h, f"imported {role} finalized CPU-h")
        with _Lock(self.path):
            state = self._read()
            prev_hash = state.get("remote_handoff_payload_hash")
            if prev_hash is not None and prev_hash != payload_hash:
                raise BudgetRefusal(
                    f"a different remote handoff ({prev_hash}) is already imported; "
                    "refusing to replace it")
            if prev_hash == payload_hash:
                return {"status": "IDEMPOTENT", "remote_cells":
                        len(state.get("remote_completed_cells", {}))}
            local = set(int(c) for c in state.get("completed_cells", {}))
            clash = local & set(cell_ids)
            if clash:
                raise BudgetRefusal(
                    f"imported remote cells collide with local completions: "
                    f"{sorted(clash)[:5]}")
            state["remote_completed_cells"] = {str(c): {"role": role} for c in cell_ids}
            state.setdefault("committed_cpu_h_by_role", {})[role] = amount
            state["remote_handoff_payload_hash"] = payload_hash
            cap = self._governed_total(state)      # fails closed if over the cap
            self._write_atomic(state)
        return {"status": "IMPORTED", "remote_cells": len(cell_ids),
                "inherited_cpu_h": amount, "cap": cap}
