...
SOURCE_CLOSURE = (
    ...
    "driver/worker_pool.py", "driver/handoff.py",
...
def preflight(auth=None, *, role=None, sys_vendor=None, env=None, live_hash=None,
    ...
    # THE ROLE-TRANSITION REPAIR: completion is the union of this host's own
    # finalised cells and any AUTHENTICATED remote completion imported from a
    # signed one-way handoff. Without the remote half, Vultr always resolved AWS
    # as active and could never schedule its own 69 cells.
    completed_all = sorted(set(status["completed_cells"])
                           | set(status.get("remote_completed_cells", [])))
    active = resolve_active_host(auth, owners, completed_all, inflight=0)
    trace.append({"gate": "concurrency_and_active_host", "status": "PASS",
                  "detail": dict(gate_concurrency(auth, resolved), **active,
                                 local_completed=len(status["completed_cells"]),
                                 remote_completed=len(status.get(
                                     "remote_completed_cells", [])),
                                 remote_handoff_payload_hash=status.get(
                                     "remote_handoff_payload_hash"),
