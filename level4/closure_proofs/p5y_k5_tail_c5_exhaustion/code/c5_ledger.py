...
ROUTES = [
    # ---------------------------------------------------------------- A. order-3 surrogate
    dict(id="A1", kill_gate='sup F/D/H -> 0', family="order-3 surrogate", mechanism="tighten the candidate sup norms sF, sD, sH feeding "
    ...
    dict(id="A2", kill_gate='fG -> 0 (perfect order-3 surrogate)', family="order-3 surrogate", mechanism="replace the zero-candidate surrogate by a real order-3 "
    ...
    dict(id="A3", kill_gate='fG -> 0 (perfect order-3 surrogate)', family="order-3 surrogate", mechanism="exploit cancellation in the identity "
    ...
    dict(id="A4", kill_gate=None, argued_without_gate=True, argument='the one real order-3 probe is at CUSUM cell 0, drift ~ 0; the tail cells are at e in [1.70, 2.09]. An order-3 value at e ~ 0 constrains nothing at e ~ 2 without a certified transport across that range, and none exists. No kill gate can express this: the quantity is absent, not large.', family="order-3 surrogate", mechanism="reuse already-certified order-3 evidence "
    ...
    # ---------------------------------------------------------------- sigma towers
    dict(id="A5", kill_gate='sigma3 -> 0', family="order-3 surrogate", mechanism="tighten sigma3, the (P3') midpoint source tower",
    ...
    dict(id="A6", kill_gate='sigma4 -> 0', family="order-3 surrogate", mechanism="tighten sigma4, the (P3) whole-cell 4th-order envelope",
    ...
    # ---------------------------------------------------------------- B. rho
    dict(id="B1", kill_gate='rho halved (cover refinement)', family="rho", mechanism="refine the cover: narrower cells, smaller rho",
    ...
    dict(id="B2", kill_gate=None, argued_without_gate=True, family="rho", mechanism="split the K5-B clause over sub-intervals of the existing cell, "
    ...
    dict(id="D1", kill_gate=None, argued_without_gate=True, family="direct K5-B", mechanism="exact-weight, sign-aware transport (theorem C5-T): replace the "
         ...
         kill_reason=None),
    dict(id="D2", kill_gate=None, argued_without_gate=True, argument='proved by exhaustive case analysis on H_lo <= H_hi: max(|H_lo|,|H_hi|) = max((H_hi)^+, (-H_lo)^+) identically, whether both ends are positive, both negative, or the interval straddles zero. Sign-awareness against a SINGLE weight is therefore exactly zero.', family="direct K5-B", mechanism="sign-awareness alone, keeping the weight rho*x_hi",
    ...
    dict(id="D3", kill_gate=None, argued_without_gate=True, family="direct K5-B", mechanism="second-order transport of g using g'(e0) = -e0 R''(e0)",
    ...
    dict(id="D4", kill_gate=None, argued_without_gate=True, family="direct K5-B", mechanism="tighten g_hi = R.hi - e0*D.lo by exploiting correlation "
    ...
    # ---------------------------------------------------------------- atom constants
    dict(id="E1", kill_gate=None, argued_without_gate=True, family="atom constants", mechanism="tighten A0 below the C3 operator-mixed value",
         ...
         status="KILLED", kill_kind="DATA", refuted_at_cells=[],
         refuted_at_cells_diagnostic_only=[309],
         diagnostic_basis="the 309 entry rests on C4's UNCERTIFIED Monte-Carlo Lambda, so it is recorded as diagnostic-only and is NOT counted as a refutation anywhere",
         ...
         kill_reason="operator certification needs python-flint, which is absent on this host; the "
                     "programme's certifying workers are out of scope for this campaign. A live "
                     "FUTURE route for 307 and 308. At cell 309 the oracle does not close, but "
                     "see refuted_at_cells_diagnostic_only: that rests on an uncertified Monte-Carlo."),
    dict(id="E2", kill_gate=None, argued_without_gate=True, family="atom constants", mechanism="certify a sharper LOWER bound on E_a[tau] than C4's "
         ...
         withdrawn_kill_reason="WITHDRAWN -- see reopened_because. The retracted v1 argument, kept for the "
                               "record and renamed so no parser reads it as live: 'zero verdict value. 309 is "
                               "already excluded by C4 and a better floor only widens a margin that is already "
                               "positive; 308 can never be excluded because the C4 adjudicator's Monte-Carlo "
                               "puts the truth (4.311) BELOW the threshold (4.375229).'"),
]
...
def main() -> int:
    ...
    # Every route declares either a kill_gate naming a knob in the sensitivity evidence, or argued_without_gate.
    # Where a gate exists the refutation sets are DERIVED from it, not compared against hand-written copies:
    # round 2 of the pre-forecast review showed that comparing two hand-written fields of the same dict cannot
    # detect a wrong one, and that an allowlist leaves every unlisted route unchecked.
    for r in ROUTES:
        g = r.get("kill_gate")
        if g is None:
            if not r.get("argued_without_gate"):
                raise SystemExit(f"route {r['id']}: no kill_gate and no argued_without_gate declaration")
            if r.get("kill_kind") == "MATH" and not r.get("argument"):
                raise SystemExit(f"route {r['id']}: a gate-free MATH kill must carry an `argument` field")
            continue
        if g not in ko:
            raise SystemExit(f"route {r['id']}: kill_gate {g!r} is not a knob in the sensitivity evidence")
        d_open = sorted(int(c) for c, v in ko[g]["cells"].items() if not v["closes"])
        d_close = sorted(int(c) for c, v in ko[g]["cells"].items() if v["closes"])
        # the gate is the source of truth. A route that also DECLARES the sets must agree with it; a route that
        # declares neither has them filled in from the gate, so no hand-written copy can drift.
        for fld, derived in (("refuted_at_cells", d_open), ("oracle_closes_at_cells", d_close)):
            if fld in r and r[fld] != derived:
                raise SystemExit(f"route {r['id']}: declared {fld} {r[fld]} disagrees with gate {g!r} "
                                 f"(derived {derived})")
            r[fld] = derived
    for r in ROUTES:
    ...
    out = {"schema": "rebaseguard.p5y.k5.tail-c5.route-ledger.v2",
           ...
           "kill_gate_backing": {r["id"]: ko[r["kill_gate"]]["cells"] for r in ROUTES if r.get("kill_gate")},
           "refuted_without_a_kill_gate": [r["id"] for r in ROUTES
                                           if r.get("kill_kind") == "MATH" and r.get("argued_without_gate")],
           "headline_caveat": "the two routes refuted on mathematics everywhere, A4 and D2, carry NO kill gate: "
                              "each is refuted by a proved argument recorded in its `argument` field, and the "
                              "producer refuses a gate-free MATH kill that does not carry one "
                              "(round-2 pre-forecast review, item C).",
