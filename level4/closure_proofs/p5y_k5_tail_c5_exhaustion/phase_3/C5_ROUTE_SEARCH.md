...
| E1 | tighten `A0` by operator certification | KILLED | DATA | needs python-flint, absent. A live future route for 307 and 308. At 309 the oracle does not close — but that rests on C4's **uncertified** Monte-Carlo, so the ledger carries it as `refuted_at_cells_diagnostic_only` and does **not** count it as a refutation. The earlier phrase “provably useless for 309” is **withdrawn**. |
