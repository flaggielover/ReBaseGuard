...
sealed records (identity gate); the new values are G_r (order-3 candidate), δ_mid(G_r), the candidate suprema and the
origins at x0.
...
- `code/tc_producer.py real` on rebaseguard-vultr-02, venv `/root/work/rbg-cusum-aux5-venv` (Python 3.12.3, numpy 2.5.2,
  scipy 1.18.1, python-flint 0.9.0), 256 bits, 1 BLAS/flint thread per process, 7 worker processes.
...
errors of orders 0..3, the candidate suprema of F̂, D̂, Ĥ, Ĝ, the origins Ĥ_r(a), Ĝ_r(a); the frozen order-2 W enclosures;
the identity-gate report; the governance binding. Plus `TC_INDEX.json`, `RUN_LEDGER.jsonl`, and after the seal
`TC_CONSUMPTION.json`.
...
- Forecast of new real CPU: see `evidence/forecast_r1/COST_NOTE.md` (measured K1 replay cost at cell 11, recorded
  before the freeze). Protocol cap: 34 CPU-hours (campaign hard cap 40 across Campaigns A and B, ≈ 4 CPU-hours reserved
  for Campaign B). `tc_run` launches no new cell once the recorded CPU would exceed the cap; that makes the run VOID.
