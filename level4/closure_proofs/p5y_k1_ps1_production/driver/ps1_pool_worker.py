...
Per task (one deterministic cell GROUP): every one of the 3,994 frozen live patches, patch-outer / cells-inner with one
...
def run_group(task: dict) -> dict:
    ...
    return {"ok": all(r["ok"] for r in results.values()) and len(results) == len(cells), "results": results,
