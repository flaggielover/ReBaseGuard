"""Calibration-block-only CUSUM threshold selection and reliability gate."""
from __future__ import annotations

import numpy as np

from config import EXECUTION, PROTOCOL, task_time
from inference import mean_summary
from monitor import run_monitor


def _measure(stream, threshold: float, r0: float) -> np.ndarray:
    ...
    run = run_monitor(stream.residual, scale=stream.scale, threshold=threshold,
                      rho=0.0, r0=r0, start=lo, stop=hi,
...
def calibrate(stream) -> dict:
    ...
    r0 = float(stream.residual[lo - m:lo].mean())
    ...
    summary = mean_summary(
        lengths, block=EXECUTION["calibration"]["cycle_block"],
        draws=PROTOCOL["bootstrap"]["draws"], seed=PROTOCOL["bootstrap"]["seed"],
    )
    floor = PROTOCOL["power"]["minimum_effective_blocks"]
    point_ok = abs(summary["mean"] / target - 1.0) <= PROTOCOL["calibration"]["point_relative_tolerance"]
    ...
    return {
        ...
        "target_arl": target, "achieved": summary, "initial_reference": r0,
        "point_relative_error": summary["mean"] / target - 1.0,
