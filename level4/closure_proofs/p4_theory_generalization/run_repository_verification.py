#!/usr/bin/env python3
"""Run and classify the repository regression suites required by Priority 4.
...
REQUIRED = (
    "priority4_focused", "level_1_3_full", "priority2",
    "d4_phase_map", "external_validation_v3", "l4r06_policy",
    "l4r12_operational", "priority1", "historical_sr", "track1b",
    "location_family_track3ab",
)

#: Suites whose integrity tests assert, by design, that *nothing outside their
#: own namespace had changed at the moment they were frozen.  Any later
#: campaign necessarily breaks that assertion by existing.  Following the
#: precedent set by ``location_family_track3ab`` for the historical Track-3
#: suite, each is run in a clean detached worktree at the revision whose tree
#: the assertion describes, where it must pass in full; the working-tree run is
#: kept as a diagnostic and its failing node ids are recorded literally.
FREEZE_SCOPED_SUITES = {
    "priority3": {
        "path": "level4/closure_proofs/m_rho_stability_priority3/tests",
        "revision": "HEAD",
        "expected_worktree_failures": [
            "level4/closure_proofs/m_rho_stability_priority3/tests/"
            "test_integrity.py::test_priority3_writes_only_inside_its_own_namespace"
        ],
    },
    "location_family": {
        "path": "level4/closure_proofs/location_family/tests",
        "revision": "1110065",
        "expected_worktree_failures": [
            "level4/closure_proofs/location_family/tests/"
            "test_integrity.py::test_fresh_seed_is_confined_to_track3_at_freeze"
        ],
    },
}
...
def pristine_head(work: Path) -> Path:
    """Materialise HEAD with no working-tree additions, Priority 4 included."""
    ...
    assert not (target / CAMPAIGN_RELATIVE).exists(), \
        "the pristine HEAD extraction must not contain any Priority-4 file"
    return target


def _detached_worktree(revision: str, target: Path) -> None:
    """Materialise a clean detached worktree and give it a venv shim.

    ``level4/.venv`` is gitignored, so a directory of that name keeps
    ``git status`` clean while making the interpreter reachable at the path the
    frozen suites expect.
    """
    subprocess.run(["git", "worktree", "add", "--detach", str(target), revision],
                   cwd=ROOT, check=True, capture_output=True)
    shim = target / "level4" / ".venv" / "bin"
    shim.mkdir(parents=True, exist_ok=True)
    (shim / "python").symlink_to(PY_BIN)


def freeze_scoped_suites(work: Path) -> dict:
    rows = {}
    for label, spec in FREEZE_SCOPED_SUITES.items():
        observed = pytest_suite(f"{label}_worktree", spec["path"])
        replays = {}
        for tag, revision in (("frozen_tree", spec["revision"]),
                              ("head_tree", "HEAD")):
            target = work / f"{tag}_{label}"
            _detached_worktree(revision, target)
            try:
                replays[tag] = pytest_suite(
                    f"{label}_{tag}", str(target / spec["path"]), cwd=target,
                )
            finally:
                subprocess.run(
        ...
        frozen = replays["frozen_tree"]
        at_head = replays["head_tree"]
        observed_nodes = {
            node.split("::", 1)[-1] for node in observed["failing_node_ids"]
        }
        expected_nodes = {
            node.split("::", 1)[-1] for node in spec["expected_worktree_failures"]
        }
        rows[label] = {
            "revision": spec["revision"],
            "working_tree": observed,
            "frozen_tree": frozen,
            "frozen_tree_passes": frozen["exit_code"] == 0,
            "clean_head_tree": at_head,
            "same_failure_already_present_at_head_without_priority4": bool(
                observed_nodes and observed_nodes <= {
                    node.split("::", 1)[-1] for node in at_head["failing_node_ids"]
                }
            ),
            "working_tree_failures_are_exactly_the_freeze_scope_assertions":
                observed_nodes == expected_nodes,
            "expected_worktree_failures": spec["expected_worktree_failures"],
            "classification": "FREEZE_SCOPED_DIAGNOSTIC",
            "reading": (
                "The assertion describes the tree at that campaign's freeze. It "
                "fails on the working tree because a later namespace exists, "
                "not because Priority 4 changed anything the campaign owns; the "
                "same suite passes in full at the revision it describes. The "
                "clean-HEAD replay separates the two possible causes: a suite "
                "that already fails at HEAD was broken by an earlier campaign "
                "and has nothing to do with Priority 4, while one that passes at "
                "HEAD is failing only because this untracked namespace exists."
            ),
        }
    rows["all_frozen_trees_pass"] = all(
        row["frozen_tree_passes"] and
        row["working_tree_failures_are_exactly_the_freeze_scope_assertions"]
        for key, row in rows.items() if key != "all_frozen_trees_pass"
    )
    return rows
...
def main() -> None:
    suites = [
        pytest_suite("priority4_focused", f"{CAMPAIGN_RELATIVE}/tests"),
        ...
        pytest_suite("location_family_track3ab",
                     "level4/closure_proofs/location_family_track3ab/tests"),
    ...
    with tempfile.TemporaryDirectory(prefix="rebaseguard-p3-pristine-") as raw:
        ...
        freeze_scoped = freeze_scoped_suites(Path(raw))
        replays = {
    ...
    for label, replay in replays.items():
        ...
        environment[label] = {
            ...
            "every_observed_failure_reproduces_without_priority4":
                bool(in_repo) and in_repo <= at_head,
            "priority4_introduced_no_new_failure": in_repo <= at_head,
            "pristine_head_summary": replay["summary"],
            "classification": "ENVIRONMENT_DIAGNOSTIC",
        }
    environment_clean = all(
        row["priority4_introduced_no_new_failure"] for row in environment.values())
    ...
    payload = {
        "schema": "rebaseguard.p4-verification.v1",
        ...
        "environment_diagnostics_reproduce_without_priority4": environment_clean,
        "historical_diagnostics": historical,
        "historical_diagnostics_unchanged": historical_unchanged,
        "freeze_scoped_suites": freeze_scoped,
        "diagnostics_not_counted_as_priority4_passes": True,
        "gate": "all named required suites literally pass in the current intended "
                "environment; every freeze-scoped suite passes in full in a clean "
                "worktree at the revision its assertion describes, and its "
                "working-tree failures are exactly the named freeze-scope "
                "assertions; controlled worktree/pristine replays prove the "
                "locale/PATH diagnostics; no observed failure is new to Priority 4; "
        ...
        "all_gates_pass": required_pass and environment_clean
            ...
            and freeze_scoped["all_frozen_trees_pass"],
    }
