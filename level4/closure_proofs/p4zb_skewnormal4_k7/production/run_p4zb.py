#!/usr/bin/env python3
"""P4ZB production driver -- four cells, one configuration, nothing else.

Reuses the P4Z estimators unchanged (bound by content hash) and the P4Z Mac
runtime contract, verified to still match this host.  Fail-closed throughout.
"""
...
sys.path.insert(0, str(HERE)); sys.path.insert(0, str(P4Z/"src"))
...
def preflight() -> dict:
    ...
    if t["fd_ladder_relative_max"] != 0.02:
        raise sh.ProducerGateError("K7 limit drift -- it must remain 0.02")
    ...
    if plan["fd_ladder"]["rungs"] != [0.1,0.05,0.025,0.0125]:
        raise sh.ProducerGateError("ladder drift")
    ...
    if plan["scope_lock"]["configuration"] != "frozen/cusum@5/skewnormal4" or \
       plan["scope_lock"]["m"] != [1,2,3,5]:
        raise sh.ProducerGateError("scope lock violated")
...
def resume_identity(ph,rh,cid,route,seed,block):
    return hashlib.sha256(f"{ph}|{rh}|{cid}|{route}|{seed}|{block}".encode()).hexdigest()


def block_path(route, block):
    return BLOCK_DIR/"frozen__cusum_at_5__skewnormal4"/f"{route}_{block:04d}.json"
...
def produce(mod, ctx, route, block):
    ...
    t_cpu,t_wall=time.process_time(),time.time(); central=None
    if route=="rb_score":
    ...
    doc={"schema":"rebaseguard.p4zb-block.v1","result_bearing":True,
         ...
         "head":ctx["head"],"cpu_seconds":cpu,"wall_seconds":time.time()-t_wall,
         "peak_rss_mb":resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/(1<<20)}
    if route=="fd_ladder":
        rungs=plan["fd_ladder"]["rungs"]
        doc["central_difference_by_h"]={f"{h:g}":{str(m):float(central[h][m].mean()) for m in m_grid} for h in rungs}
        doc["richardson_by_pair"]={f"{c:g}/{f:g}":{str(m):float(((4.0*central[f][m]-central[c][m])/3.0).mean()) for m in m_grid}
...
def read_blocks(route):
    d=block_path(route,0).parent
    return [json.loads(p.read_text()) for p in sorted(d.glob(f"{route}_*.json"))] if d.exists() else []


def main() -> int:
    ap=argparse.ArgumentParser(); ap.add_argument("--stage",choices=("a","full"),required=True)
    args=ap.parse_args()
    ctx=preflight(); mod=load_modules(); sh.final_producer_gate(ctx["manifest"])
    plan=ctx["plan"]; cfg=plan["configuration"]
    state=json.loads(STATE.read_text()) if STATE.exists() else {
        "schema":"rebaseguard.p4zb-run-state.v1","cpu_seconds_total":0.0,
        "calibration_cpu_seconds":plan["budget"]["ladder_study_cpu_seconds"]}
    cap=plan["budget"]["total_cpu_cap_hours"]*3600
    if args.stage=="a":
        spans={"rb_score":range(0,20),"rb_map":range(0,20),"fd_ladder":range(0,20)}
    else:
        if state.get("stage_a_verdict")!="P4ZB_STAGE_A_PASS":
            raise sh.ProducerGateError(f"full run requires P4ZB_STAGE_A_PASS; have {state.get('stage_a_verdict')!r}")
        spans={"rb_score":range(0,cfg["blocks"]),"rb_map":range(0,cfg["blocks"]),
               "fd_ladder":range(0,cfg["ladder_blocks"])}
    t0=time.process_time()
    for route,span in spans.items():
        for b in span:
            _,cpu=produce(mod,ctx,route,b)
            state["cpu_seconds_total"]+=cpu
            total=state["cpu_seconds_total"]+state["calibration_cpu_seconds"]
            if total>cap:
                state["verdict"]="K8_TOTAL_CPU_CAP"
                STATE.write_text(json.dumps(state,indent=2,sort_keys=True)+"\n")
                raise SystemExit("K8: total CPU cap reached; STOP")
        print(f"  {route:10s} {len(list(span))} blocks  run={state['cpu_seconds_total']/3600:6.4f}h "
              f"+calib={state['calibration_cpu_seconds']/3600:6.4f}h", flush=True)
        STATE.write_text(json.dumps(state,indent=2,sort_keys=True)+"\n")
    print(f"stage {args.stage} done in {(time.process_time()-t0)/3600:.4f} CPU-h")
    STATE.write_text(json.dumps(state,indent=2,sort_keys=True)+"\n")
    return 0
