...
def validate_record(rec: dict, role: str, owners: dict[int, str],
    ...
    if isinstance(cs, bool) or not isinstance(cs, (int, float)) \
            or not math.isfinite(cs) or cs < 0:
...
def gate_global_cap(cpu_h_by_role: dict[str, float],
    ...
    """
    for role, v in cpu_h_by_role.items():
        if not isinstance(v, (int, float)) or not math.isfinite(v) or v < 0:
            raise MultiHostRefusal(f"non-physical governed CPU-h for {role}: {v!r}")
...
def reconcile_accounting(host_accounts: dict[str, dict],
    ...
    for role, recs in host_ledgers.items():
        ...
        claimed = acct.get("cpu_h")
        if not isinstance(claimed, (int, float)) or not math.isfinite(claimed):
            raise MultiHostRefusal(f"corrupted accounting: {role} cpu_h {claimed!r}")
